...
int (*tmrSchedWrapper)(int entryId, int time) = NULL;
int (*tmrCanWrapper)(int entryId) = NULL;
...
PJ_DEF(pj_status_t) timer_android_setcallbacks(int(*schWrapper)(int entryId, int time),
                                      int(*canWrapper)(int entryId)) {
    tmrSchedWrapper = schWrapper;
    tmrCanWrapper = canWrapper;
    return 0;
}
...
// Find one empty slot in ht entries
	...
	PJ_LOG(
			5,
			(THIS_FILE, "Scheduling _timer_id %d heap %d in %ld ms @ %p", entry->_timer_id, ht->heap_id, ft, entry));

	if (tmrSchedWrapper) {
		(*tmrSchedWrapper)(get_entry_id(ht, entry), (int)ft);
	}
	// timer_schedule_wrapper((int) entry, get_entry_id(ht, entry), (int) ft);

	return PJ_SUCCESS;
    } else {
	PJ_LOG(3, (THIS_FILE, "ScheduleEntry; All slots full"));
	return PJ_ETOOMANY;
    }
}
...
//		PJ_LOG(
//				3,
//				(THIS_FILE, "Ask to cancel something already fired or canceled : %d @ %p", entry->_timer_id, entry));
		return 0;
	...
	if (ht->entries[entry->_timer_id - 1] != entry) {
        PJ_LOG(3,
            (THIS_FILE, "Canceling something not linked to this heap : %d : %p vs %p", entry->_timer_id,
	...
	// Previous checks should do the trick to be sure we have actually 1 canceled timer here.
	int cancelCount = 0;
	if (tmrCanWrapper) {
		cancelCount = (*tmrCanWrapper)(get_entry_id(ht, entry));
	}
	// int cancelCount = timer_cancel_wrapper((int) entry, get_entry_id(ht, entry));

	if (cancelCount <= 0) {
		PJ_LOG(4, (THIS_FILE, "Cancel timer_id %d cancelCount=", entry->_timer_id, cancelCount));
	}
...
#endif
{
    ...
    PJ_LOG(4,(THIS_FILE, "schedule_w_grp_lock: delayMs=%d heap_id=%d _timer_id=%d grp_lock=%x",
    		PJ_TIME_VAL_MSEC(*delay), ht->heap_id, entry->_timer_id, grp_lock));

#if PJ_TIMER_DEBUG
    entry->src_file = src_file;
    entry->src_line = src_line;
    PJ_LOG(4,(THIS_FILE, "schedule_w_grp_lock: source=%s:%d", src_file, src_line));
#endif
            ...
            int id_val)
{
    ...
    PJ_LOG(5,(THIS_FILE, "cancel_timer request: _timer_id=%d", entry->_timer_id));
...
PJ_DEF(void) pj_timer_heap_dump(pj_timer_heap_t *ht)
{
    ...
    for (i = 0; i < MAX_ENTRY_PER_HEAP; i++) {
        ...
        if (!e) {
        	PJ_LOG(3,(THIS_FILE, "    Null \t\t\t%d", ht->entries_reuse_in[i]));
        	continue;
        }
...
PJ_DEF(pj_status_t) pj_android_timer_fire(int entryId) {
    pj_thread_desc a_thread_desc;
	pj_thread_t *a_thread;
	int entry_id, heap_id;
	pj_timer_entry *entry;

	entry_id = entryId % MAX_ENTRY_PER_HEAP;
	heap_id = entryId / MAX_ENTRY_PER_HEAP;
	PJ_LOG(4, (THIS_FILE, "FIRE timer code %d  heap %d", entryId, heap_id));

	if (heap_id < 0 || heap_id >= MAX_HEAPS){
		PJ_LOG(1, (THIS_FILE, "Invalid timer code %d", entryId));
		return PJ_EINVAL;
	}
	...
	if (!pj_thread_is_registered()) {
		...
		int len = pj_ansi_snprintf(thread_name, sizeof(thread_name),
				"timer_thread_%d", entryId);
		thread_name[len] = '\0';
		pj_thread_register(thread_name, a_thread_desc, &a_thread);
		PJ_LOG(4, (THIS_FILE, "Registered thread %s", thread_name));
	}
	...
	if (ht != NULL) {
	    ...
	    PJ_LOG(5, (THIS_FILE, "FIRE entry_id %d of heap %d", entry_id, heap_id));
        ...
        if (cb) {
        	PJ_LOG(5, (THIS_FILE, "FIRE Executing callback. User value id=%d", entry->id));
            cb(ht, entry);
        } else {
        	PJ_LOG(4, (THIS_FILE, "FIRE No callback vector"));
        }
		...
		PJ_LOG(3, (THIS_FILE, "FIRE Ignore : No heap found in heap %d for this entryId %d", heap_id, entryId));
	}
